...
class PrescriptiveEngine:
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            print("[PrescriptiveEngine] Loading Embedding Model (this may download on first run)...")
            ...
            # 4. Setup LLM (Mistral)
            if self.settings.mistral_api_key and self.settings.mistral_api_key != "your_mistral_api_key_here":
                self.llm = ChatMistralAI(
                    mistral_api_key=self.settings.mistral_api_key,
                    model=self.settings.mistral_model
                )
                print(f"[PrescriptiveEngine] LLM initialized: Mistral AI ({self.settings.mistral_model})")
            else:
                print("[PrescriptiveEngine] No Mistral API key found. Running in Retrieval-Only mode.")
    ...
    def query_protocols(self, query_text: str, n_results: int = 3) -> List[SourceDocument]:
        """
    ...
    async def chat_rag(self, user_query: str, district_context: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    # 2. LLM Synthesis
        ...
        if self.llm:
            try:
                ...
                messages = [
                    ("system", system_prompt.format(context=context_str, extra_context=extra_context_block)),
                    ("user", user_query)
                ]
                ...
                answer = "I encountered an error processing your request with the AI model."
    ...
    async def generate_prescriptive_advice(
        ...
        context_str = "\n\n".join([f"Source ({d.source}): {d.content}" for d in context_docs])
        ...
        if self.llm:
            try:
                ...
                print(f"[PrescriptiveEngine] LLM Generation failed: {e}. Falling back to rule-based.")
