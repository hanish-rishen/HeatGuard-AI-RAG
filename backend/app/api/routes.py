...
async def health_check():
    ...
    return HealthCheckResponse(
        ...
        chroma_connected=prescriptive_engine.collection is not None,
...
async def delete_file(filename: str):
    ...
    try:
        ...
        return {"filename": filename, "status": "deleted", "message": "File deleted successfully"}

    except Exception as e:
        logger.error(f"Delete failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))

def process_document_background(filename: str, content: bytes, content_type: str, doc_id: str):
    """
    ...
    try:
        ...
        for i, chunk in enumerate(text_chunks):
            if i > 0 and i % 5 == 0:
                 db_manager.update_file_status(filename, f"Indexing {i}/{len(text_chunks)}")
...
async def analyze_district(request: AnalysisRequest):
    ...
    if heat_index > 45 or pred_load > 100:
        ...
        query_context = f"critical heatwave protocols severe heat stroke hospitalization"
...
async def get_district_rankings():
    ...
    async def event_generator():
        try:
            ...
            existing_names = set(r['district_name'] for r in existing_results) if existing_results else set()
            ...
            for i, district_name in enumerate(districts_to_analyze):
                try:
                    ...
                    risk_status = "Red" if pred_load > 0.8 else "Amber" if pred_load > 0.5 else "Green"
            ...
            coord_hits = sum(1 for r in results if r.get("lat") is not None and r.get("lon") is not None)
